       ...
       (finally
         (let [log-contents (slurp log-file)]
           (when-not (str/blank? log-contents)
             (println "-------Begin PuppetDB Instance Log--------------------\n"
                      log-contents
...
(defn metrics-url
  "Returns the URL string for v1 metrics built on `base-url`"
  [base-url]
  (utils/base-url->str (assoc base-url :prefix "/metrics" :version :v1)))

(defn dlo-count
  "Return the queue depth currently running PuppetDB instance (see `puppetdb-instance` for launching PuppetDB)"
  []
  (let [discarded-count (-> (metrics-url *base-url*)
                            (str "/mbeans/puppetlabs.puppetdb.command:type=global,name=discarded")
                            (client/get {:as :json})
                            (get-in [:body :Count]))
        fatal-count (-> (metrics-url *base-url*)
                        (str "/mbeans/puppetlabs.puppetdb.command:type=global,name=fatal")
                        (client/get {:as :json})
                        (get-in [:body :Count]))]
    (+ fatal-count discarded-count)))

(defn current-queue-depth
  "Return the queue depth currently running PuppetDB instance (see `puppetdb-instance` for launching PuppetDB)"
  []
  (-> (str (metrics-url *base-url*)
